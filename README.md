...
Static Astro + React app for building a local Deadlock `topbar_rank` VPK. The app runs in the browser: users upload the exact Top Bar Plus v40c and ShowRank 2026-07-07 normal archives, validation happens locally, the current combined `topbar_rank` source is fetched from GitHub at build time, and the generated VPK downloads to the user's machine.

Hosted app: <https://hantu-raya.github.io/Show-rank-merger/>

## What it does

- Validates the exact Top Bar Plus v40c and ShowRank 2026-07-07 normal GameBanana archives and embedded VPKs.
...
Filenames are only hints. Archive size, archive SHA-256, embedded VPK SHA-256, and required VPK paths decide compatibility. The current Topbar Rank integration supports its normal build.
...
Verify the live Top Bar Plus v40c and ShowRank fixtures and generated 19-resource VPK:
